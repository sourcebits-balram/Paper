...
- (void)oneFingerGesture:(UIPanGestureRecognizer *)sender
{
    ...
    if (self.hasActiveInteraction) {
        if ([sender state] == UIGestureRecognizerStateChanged)
        {
            // UICollectionViewCell *piece  = [self.collectionView cellForItemAtIndexPath:[self.collectionView indexPathForItemAtPoint:point]];
            CGPoint point1 = self.initialPanPoint;
...
//                [sender setTranslation:CGPointZero inView:piece];
//
            } completion:^(BOOL finished) {
                
                CGFloat distance = sqrt((point1.x - point2.x) * (point1.x - point2.x) + (point1.y - point2.y) * (point1.y - point2.y));
                CGRect collectionViewFrame = self.collectionView.frame;
                progress =  MIN(1.0, MAX(0.0, distance/self.collectionView.frame.size.height/2.0f));
                NSLog(@"Distance::%f, CollectionHeight::%f, Progress::%f",distance, collectionViewFrame.size.height,  progress);
